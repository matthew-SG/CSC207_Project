...
/**
 * Strategy for meal plan generator that prioritizes having the error between the target calories and the meal plan
 *      calories being as low as possible.
 */
public class PrioritizeCaloriesErrorStrategy implements MealPlanGeneratorStrategy {
    ...
    public List<Recipe> generateMealPlan(List<List<Recipe>> recipeTriplets, double targetCalories, double targetProtein,
        ...
        // The weight that the calories have on the error
        final int caloriesWeight = 3;
        ...
        for (List<Recipe> recipeTriplet : recipeTriplets) {
            ...
            currentError = caloriesWeight * Math.abs(currentCalories - targetCalories)
                    + Math.abs(currentProtein - targetProtein) + (Math.abs(currentCarbs - targetCarbs)
