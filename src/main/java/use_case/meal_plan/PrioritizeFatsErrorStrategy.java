...
/**
 * Strategy for meal plan generator that prioritizes having the error between the target fats and the meal plan
 *      fats being as low as possible.
 */
public class PrioritizeFatsErrorStrategy implements MealPlanGeneratorStrategy {
    @Override
    public List<Recipe> generateMealPlan(List<List<Recipe>> recipeTriplets, double targetCalories, double targetProtein,
        ...
        // The weight that the protein have on the error
        final int fatsWeight = 3;
        ...
        for (List<Recipe> recipeTriplet : recipeTriplets) {
            ...
            currentError = Math.abs(currentCalories - targetCalories)
                    ...
                    + fatsWeight * Math.abs(currentFats - targetFats));
            if (currentError == 0) {
