...
/**
 * Strategy for meal plan generator that prioritizes having the error between the target protein and the meal plan
 *      protein being as low as possible.
 */
public class PrioritizeProteinErrorStrategy implements MealPlanGeneratorStrategy {
    @Override
    public List<Recipe> generateMealPlan(List<List<Recipe>> recipeTriplets, double targetCalories, double targetProtein,
        ...
        for (List<Recipe> recipeTriplet : recipeTriplets) {
            ...
            currentError = Math.abs(currentCalories - targetCalories)
                    + proteinWeight * Math.abs(currentProtein - targetProtein) + (Math.abs(currentCarbs - targetCarbs)
